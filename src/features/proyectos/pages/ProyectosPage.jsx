...
const getNextProyectoCode = (proyectos = []) => {
  const maxNumero = proyectos.reduce((max, proyecto) => {
    const codigo = String(proyecto?.codigo ?? "").toUpperCase().trim();
    const match = codigo.match(/^PRY-(\d+)$/);
  ...
  return `PRY-${String(maxNumero + 1).padStart(3, "0")}`;
};
...
const ProyectosPage = () => {
  ...
  const nextProyectoCode = getNextProyectoCode(proyectos);
          ...
          {proyectosFiltrados.map((proyecto) => {
            ...
            const intervOld =
              actsProyecto.length === 0
                ? Object.entries(proyecto.actividades_por_intervencion ?? {}).filter(
                ...
                <div className="proy-card-footer">
                  ...
                  <div className="proy-acciones">
                    <button
                      ...
                      title="Subproyectos"
                      onClick={() =>
                        navigate(`/proyectos/subproyectos?proyecto=${proyecto._id}`)
                      }
                      style={{
                        background: "#f0fdf4",
