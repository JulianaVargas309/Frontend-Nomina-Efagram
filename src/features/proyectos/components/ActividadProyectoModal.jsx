...
//   onClose        {function}
//   onSuccess      {function}
//   proyecto       {object}    — proyecto padre (obligatorio)
//   actividad      {object|null} — null = crear, objeto = editar
// ══════════════════════════════════════════════════════════
const ActividadProyectoModal = ({
  ...
  const total =
    (Number(form.precio_unitario) || 0) * (Number(form.cantidad_total) || 0);

  // ── Cargar catálogo, clientes y personas al abrir ──
  useEffect(() => {
    if (!isOpen) return;
    ...
    if (isEdit && actividad) {
      setForm({
        ...
        intervencion:    actividad.intervencion    ?? "mantenimiento",
      ...
      setForm({
        actividad_id: "", intervencion: "mantenimiento",
        cliente: "", supervisor: "",
  ...
  const col = INTERVENCION_COLOR[form.intervencion] ?? {};
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div
              style={{
                ...
                background: col.light ?? "#e8f5ee",
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 22,
              }}
            >
              {TIPO_EMOJI[form.intervencion] ?? "📦"}
            </div>
            <div>
          ...
          <button
            onClick={onClose}
            style={{
              background: "none", border: "1px solid #e6e8ef", borderRadius: 8,
              width: 32, height: 32, display: "flex", alignItems: "center",
              justifyContent: "center", cursor: "pointer", fontSize: 18,
              color: "#64748b", lineHeight: 1, flexShrink: 0,
            }}
          >
            ×
          </button>
        ...
        <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {["mantenimiento", "no_programadas", "establecimiento"].map((tipo) => {
                const c = INTERVENCION_COLOR[tipo];
                const sel = form.intervencion === tipo;
                return (
                  <button
                    key={tipo}
                    type="button"
                    onClick={() => setForm((p) => ({ ...p, intervencion: tipo }))}
                    style={{
                      flex: 1,
                      padding: "9px 8px",
                      border: sel ? `2px solid ${c.border}` : "1.5px solid #e2e8f0",
                      borderRadius: 9,
          ...
          <div>
            ...
            {loadData ? (
              ...
              <select
                ...
                style={{ ...inputStyle, border: errors.actividad_id ? "1.5px solid #dc2626" : "1.5px solid #d1d5db", cursor: "pointer" }}
              >
                <option value="">— Selecciona una actividad —</option>
                {catalogo.map((a) => (
                  <option key={a._id} value={a._id}>
                    {a.nombre} ({a.unidad_medida}) — {a.codigo}
                  </option>
                ))}
              </select>
            )}
            {errors.actividad_id && (
              <p style={{ margin: "4px 0 0", fontSize: 12, color: "#dc2626" }}>{errors.actividad_id}</p>
            )}
            {/* Preview de la actividad seleccionada */}
            {actSeleccionada && (
              <div
                style={{
                  marginTop: 8, padding: "8px 12px",
                  background: col.bg ?? "#f0faf4",
                  border: `1px solid ${col.border ?? "#1f8f57"}33`,
                  borderRadius: 8, fontSize: 12, color: col.color ?? "#1f8f57",
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              <label style={labelStyle}>
                <Hash size={11} style={{ marginRight: 4 }} />
                Cantidad total *
              </label>
              <input
                type="number"
                ...
                onChange={handleChange}
                style={{ ...inputStyle, border: errors.cantidad_total ? "1.5px solid #dc2626" : "1.5px solid #d1d5db" }}
              />
              {errors.cantidad_total && (
                <p style={{ margin: "4px 0 0", fontSize: 12, color: "#dc2626" }}>{errors.cantidad_total}</p>
              )}
              {actSeleccionada && form.cantidad_total && (
                <p style={{ margin: "4px 0 0", fontSize: 11, color: "#64748b" }}>
                  {form.cantidad_total} {actSeleccionada.unidad_medida}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                style={{ ...inputStyle, border: errors.precio_unitario ? "1.5px solid #dc2626" : "1.5px solid #d1d5db" }}
              />
              {errors.precio_unitario && (
                <p style={{ margin: "4px 0 0", fontSize: 12, color: "#dc2626" }}>{errors.precio_unitario}</p>
              )}
            </div>
          </div>
          ...
          {total > 0 && (
            ...
            >
              ...
              <span style={{ fontSize: 20, fontWeight: 900, color: "#1f8f57" }}>
                {fmtMoney(total)}
              </span>
            </div>
          )}
          ...
          {isEdit && actividad?.estado === "CERRADA" && (
            ...
            >
              🔒 Esta actividad está <strong>CERRADA</strong> — el 100% de la cantidad ha sido asignada a subproyectos.
              Solo puedes aumentar la cantidad total.
            </div>
          )}
          {isEdit && (
            <div
        ...
        >
          <button
            ...
            style={{
              ...
              padding: "10px 20px", borderRadius: 9, fontWeight: 700,
