...
import {
  ...
  FileText, Pencil, DollarSign, MapPin, PlusCircle, AlertCircle
} from "lucide-react";
...
const ProyectoModal = ({ isOpen, onClose, onSuccess, proyecto = null, modo = "crear" }) => {
    ...
    if ((modoEditar || modoVer) && proyecto) {
      ...
      ["mantenimiento", "no_programadas", "establecimiento"].forEach((tipo) => {
        (api[tipo] ?? []).forEach((act) => {
          let bloque = bloquesMigrados.find((b) => b.tipo === tipo && !b._migrated_closed);
          if (!bloque) {
            bloque = {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (Array.isArray(backendErrors) && backendErrors.length > 0) {
        ...
        const msg = err?.response?.data?.message ?? err?.message ?? "Error guardando el proyecto.";
        setFormErrors([msg]);
      }
    } finally {
      setLoading(false);
  ...
  if (modoVer && proyecto) {
    ...
    const interEntries = Object.entries(apiInter).filter(([, arr]) => Array.isArray(arr) && arr.length > 0);
    ...
    return (
      <div className="modal-overlay">
        <div style={{ width: "min(680px, calc(100% - 24px))", background: "#fff", border: "1px solid #e6e8ef", borderRadius: 18, boxShadow: "0 24px 64px rgba(15,23,42,0.22)", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
          <div style={{ padding: "22px 24px 18px", borderBottom: "1px solid #f0f2f5", display: "flex", alignItems: "flex-start", gap: 14, flexShrink: 0 }}>
            ...
            <button onClick={onClose} style={{ background: "#f8fafc", border: "1px solid #e6e8ef", borderRadius: 8, width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0, fontSize: 18, color: "#64748b", lineHeight: 1 }}>×</button>
          </div>

          <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 20 }}>
            <div style={{ background: "#f8fafc", borderRadius: 12, padding: "16px 20px", border: "1px solid #e6e8ef" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                ...
                <span style={{ fontSize: 22, fontWeight: 900, color: avance >= 80 ? "#1f8f57" : avance >= 40 ? "#e67e22" : "#0f172a" }}>{avance}%</span>
              </div>
                    ...
                    return (
                      <div key={tipo} style={{ background: col.bg, border: `1.5px solid ${col.border}`, borderRadius: 12, padding: "14px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                          <span style={{ fontSize: 14, fontWeight: 700, color: col.color }}>{TIPO_EMOJI[tipo]} {tipo.replace(/_/g, " ").charAt(0).toUpperCase() + tipo.replace(/_/g, " ").slice(1)}</span>
                          <span style={{ fontSize: 13, color: col.color, fontWeight: 600 }}>{acts.length} actividad{acts.length !== 1 ? "es" : ""}{pres?.monto_presupuestado ? ` · ${fmtMonto(pres.monto_presupuestado)}` : ""}</span>
                        </div>
                          ...
                          {acts.map((act, i) => (
                            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#fff", borderRadius: 8, padding: "8px 12px", fontSize: 13 }}>
                              ...
                              <div style={{ display: "flex", gap: 12, color: "#64748b", flexShrink: 0 }}>
                                ...
                                {act.precio_unitario > 0 && act.cantidad > 0 && <span style={{ fontWeight: 700, color: col.color }}>= {fmtMonto(act.precio_unitario * act.cantidad)}</span>}
                              </div>
                            </div>
                          ))}
                        </div>
            ...
            {interEntries.length === 0 && <p style={{ textAlign: "center", color: "#94a3b8", fontSize: 14, margin: 0 }}>Este proyecto no tiene intervenciones registradas.</p>}
          </div>

          <div style={{ padding: "16px 24px", borderTop: "1px solid #f0f2f5", display: "flex", justifyContent: "flex-end", gap: 10, flexShrink: 0 }}>
            ...
            <button onClick={() => onSuccess?.("editar")} style={{ background: "#1f8f57", color: "#fff", border: "none", padding: "10px 20px", borderRadius: 10, fontWeight: 700, cursor: "pointer", fontSize: 14, display: "flex", alignItems: "center", gap: 8, boxShadow: "0 4px 12px rgba(31,143,87,0.25)" }}>
  ...
  return (
    <div className="modal-overlay">
      <div className="modal modal-proyecto" style={{ maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ paddingRight: "8px" }}>
            ...
            <div className="form-group">
              ...
              <select name="responsable" value={form.responsable} onChange={handleChange} disabled={loadingData}>
                ...
                {personas.map((p) => (
                  <option key={p._id} value={p._id}>{`${p.nombres ?? ""} ${p.apellidos ?? ""}`.trim() || p.nombre || "Persona"}</option>
                ))}
              </select>
            </div>

            <div className="form-group">
              <label style={{ display: "flex", alignItems: "center", gap: 6 }}><MapPin size={13} /> Zona *</label>
              <select name="zona" value={form.zona} onChange={handleChange} disabled={loadingData}>
                <option value="">{loadingData ? "Cargando..." : "— Seleccione una zona —"}</option>
                {zonas.map((z) => (
                  <option key={z._id} value={z._id}>{z.nombre} {z.codigo ? `(${z.codigo})` : ""}</option>
                ))}
              </select>
              <p style={{ margin: "4px 0 0", fontSize: 12, color: "#64748b" }}>La zona determina los núcleos disponibles para los subproyectos.</p>
            </div>
